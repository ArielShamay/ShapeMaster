...
public class Point2D{
	...
	public void scale(Point2D cen, double ratio) {
		_x = (_x - cen.x()) * ratio + cen.x();
		_y = (_y - cen.y()) * ratio + cen.y();
		
	}
	public void rotate(Point2D cen, double angleDegrees)
	{
		double newX = (_x - cen.x()) * Math.cos(angleDegrees) - (_y - cen.y()) * Math.sin(angleDegrees) + cen.x();
		double newY = (_x - cen.x()) * Math.sin(angleDegrees) + (_y - cen.y()) * Math.cos(angleDegrees) + cen.y();
		_x = newX;
		_y = newY;
	}
}
